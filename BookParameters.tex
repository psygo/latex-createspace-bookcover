...
\pgfmathsetmacro\TotalPageCount{160}% Must be manually entered
...
\def\TheMainTitle{Como Jogar Go}
\def\TheSubTitle{Uma Introdução Concisa}
\def\TheSubTitleA{}
\def\TheSubTitleB{Uma Introdução Concisa}
\def\TheBookSeries{Books That Are Really Odd Series}

\def\TheAuthor{Bozulich Davies Fanaro}
\def\TheAuthorA{Bozulich}
\def\TheAuthorB{Davies}
\def\TheAuthorC{Fanaro}
\def\TheAuthorLNF{Bozulich, Richard}

\def\TheCopyrightYear{2017}
\def\TheEdition{First Edition}

\def\ThePublisherName{Kiseido}% Just the short form name
